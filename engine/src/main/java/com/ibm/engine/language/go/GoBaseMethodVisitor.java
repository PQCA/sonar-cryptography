...
public class GoBaseMethodVisitor implements IBaseMethodVisitor<Tree> {
    ...
    public void visitMethodDefinition(@Nonnull Tree method) {
        if (method instanceof FunctionDeclarationTree functionDeclarationTree) {
            functionDeclarationTree.accept(this);
        }
    }

    // @Override
    public void visitMethodInvocation(@Nonnull FunctionInvocationTree tree) {
