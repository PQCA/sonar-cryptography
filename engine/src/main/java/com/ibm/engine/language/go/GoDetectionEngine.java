...
public final class GoDetectionEngine implements IDetectionEngine<Tree, Symbol> {
    // private static final Logger LOGGER = LoggerFactory.getLogger(GoDetectionEngine.class);

    @Nonnull private final DetectionStore<GoCheck, Tree, Symbol, InputFileContext> detectionStore;
    ...
    public GoDetectionEngine(
            ...
            @Nonnull Handler<GoCheck, Tree, Symbol, InputFileContext> handler) {
    ...
    public void run(@Nonnull TraceSymbol<Symbol> traceSymbol, @Nonnull Tree tree) {
        if (tree instanceof FunctionInvocationTree functionInvocationTree) {
            handler.addCallToCallStack(functionInvocationTree, detectionStore.getScanContext());
            if (detectionStore
                    .getDetectionRule()
                    .match(functionInvocationTree, handler.getLanguageSupport().translation())) {
                this.analyseExpression(traceSymbol, functionInvocationTree);
            }
        }
    }
    ...
    public Tree extractArgumentFromMethodCaller(
        ...
        if (methodDefinition instanceof FunctionDeclarationTree functionDeclarationTree
                && methodInvocation instanceof FunctionInvocationTree functionInvocationTree
                && methodParameterIdentifier instanceof IdentifierTree identifierTree) {
            if (functionDeclarationTree.formalParameters().size()
                    != functionInvocationTree.arguments().size()) {
                return null;
            }
            ...
            List<Symbol> argsVarIds = functionInvocationTree.methodSymbol().declarationParameters();
    ...
    private <O> List<ResolvedValue<O, Tree>> resolveValues(
            @Nonnull Class<O> clazz,
            @Nonnull ExpressionStatementTree tree,
            @Nullable IValueFactory<Tree> valueFactory,
    ...
    private void analyseExpression(
            @Nonnull TraceSymbol<Symbol> traceSymbol, @Nonnull Tree expressionTree) {
        /*
        ...
        if (expressionTree instanceof FunctionInvocationTree functionInvocationTree
                && (functionInvocationTree.memberSelect()
                        instanceof MemberSelectTree memberSelectExpressionTree)) {
            isBuilderPattern = true;
        ...
        if (expressionTree instanceof FunctionInvocationTree functionInvocationTree) {
            isInvocation = isInvocationOnVariable(functionInvocationTree, traceSymbol);
